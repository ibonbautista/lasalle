...
CREATE TABLE IF NOT EXISTS `ClubDeportivoLaSalle`.`matches` (   `match_id` INT NOT NULL AUTO_INCREMENT,   `season_id` INT NOT NULL,   `team_id` INT NOT NULL,   `rival_id` INT NOT NULL,   `date` DATETIME NOT NULL,   `home_or_away` ENUM('home', 'away') NOT NULL,   `home_score` INT UNSIGNED NULL,   `away_score` INT UNSIGNED NULL,   `general_comment` VARCHAR(65535) NULL,   PRIMARY KEY (`match_id`),   INDEX `fk_matches_teams1_idx` (`team_id` ASC) VISIBLE,   INDEX `fk_matches_rivals1_idx` (`rival_id` ASC) VISIBLE,   INDEX `fk_matches_seasons1_idx` (`season_id` ASC) VISIBLE,   CONSTRAINT `fk_matches_teams1`     FOREIGN KEY (`team_id`)     REFERENCES `ClubDeportivoLaSalle`.`teams` (`team_id`)     ON DELETE NO ACTION     ON UPDATE NO ACTION,   CONSTRAINT `fk_matches_rivals1`     FOREIGN KEY (`rival_id`)     REFERENCES `ClubDeportivoLaSalle`.`rivals` (`rival_id`)     ON DELETE NO ACTION     ON UPDATE NO ACTION,   CONSTRAINT `fk_matches_seasons1`     FOREIGN KEY (`season_id`)     REFERENCES `ClubDeportivoLaSalle`.`seasons` (`season_id`)     ON DELETE NO ACTION     ON UPDATE NO ACTION) ENGINE = InnoDB
